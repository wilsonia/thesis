...
\begin{tikzpicture}[shorten >=1pt,auto, thick,
    ...
    \node(w0) at (-0.5,0) {$\ket{\psi_s} \otimes \ket{\varnothing_D}$};
    \node[wf] (w1) at (4.75, 0.5) {$P^z_+\ket{\psi_s} \otimes \ket{+_D}$};
    \node[wf] (w2) at (4.75, -0.5) {$P^z_-\ket{\psi_s} \otimes \ket{-_D}$};
...
Notice that the final sum does not contain any terms representing incorrect correlations between spin and pointer states (such as $ P^z_+ \ket{\psi_s} \otimes \ket{-_D}$).  Consequently, the final state cannot be written as the tensor product of a state in $\mathcal{H}_s$ and a state in $\mathcal{H}_D$ (as the inital state was). This is the definition of entanglement; the von Neumann measurement scheme describes measurement as entanglement of the measured system and the apparatus.
...
\begin{tikzpicture}[shorten >=1pt,auto, thick,
    ...
    \node(w0) at (0.75,0) {$\ket{\psi_s} \otimes \ket{\varnothing_{D_1}} \otimes \ket{\varnothing_{D_2}} $};
    \node[wf] (w1) at (7.25, -.5) {$ P^z_-\ket{\psi_s} \otimes \ket{-_{D_1}} \otimes \ket{\varnothing_{D_2}}$};
    \node[wf] (w2) at (10, 1) {$P^x_-P^z_+\ket{\psi}_s \otimes \ket{\varnothing_{D_1}} \otimes \ket{-_{D_2}}$};
    \node[wf] (w3) at (10, 2) {$P^x_+ P^z_+\ket{\psi_s} \otimes \ket{\varnothing_{D_1}} \otimes \ket{+_{D_2}}$};
